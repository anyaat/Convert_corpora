...
n = 0
pathIn = './dialect/texts'

def get_meta_dialect(file_name):
    stops = ['tagging']
    meta_all = ['header', 'author', 'sex', 'created', 'words', 'sphere', 'style', 'birthday', 'clausula', 'meter',\
               'audience_age', 'audience_level', 'audience_size', 'source', 'subcorpus', 'topic', 'not_place']
...
with open('dialect.txt', 'w', encoding='utf-8') as file:
    for root, dirs, files in os.walk(pathIn):
        for fileIn in files:
            ...
            metas = get_meta_dialect(file_xhtml)
            metas['docid'] = str(n)
            first_row = '<doc'
            for meta in metas:
            ...
            with codecs.open(file_xhtml, encoding='windows-1251') as f:
                for line in f:
